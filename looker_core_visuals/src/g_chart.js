 ...
 * g_chart.js
 * ===========
 * NHS Making Data Count g Chart for Looker Core
 *
 * Implements the g (geometric) chart following NHS MDC methodology and
 * R. Lloyd, *Quality Health Care*, Chapter 9.  g charts are used for
 * **rare events** where the value is the number of opportunities (or
 * cases) between successive events.  The geometric distribution gives:
 *
 *   CL  = mean(values) = g_bar
 *   σ   = sqrt(g_bar * (g_bar + 1))
 *   UCL = g_bar + 3σ
 *   LCL = max(0, g_bar - 3σ)   (typically 0)
 */
...
/**
 * Calculate g chart control limits following NHS MDC methodology
 * @param {Array} data - Array of data objects
 * @param {string} valueColumn - Name of count column
 * @returns {Object} - Calculated limits and processed data
 */
export function calculateGChartLimits(data, valueColumn = 'value') {
  validateData(data, 'g', valueColumn);

  const counts = data.map(row => Number(row[valueColumn]));
  if (counts.some(c => c < 0)) {
    throw new Error('g-chart values must be non-negative counts');
  }

  // Centre line: mean opportunities between events
  const meanCount = calculateMean(counts);

  // Geometric-distribution standard deviation
  const standardError = Math.sqrt(meanCount * (meanCount + 1));
...
export const GChart = {
  ...
  options: {
    value_column: {
      type: 'string',
      label: 'Value Column (Count Between Events)',
      default: 'value',
      section: 'Data',
      order: 1
    },
    chart_title: {
      type: 'string',
      label: 'Chart Title',
      default: 'g Chart - Opportunities Between Rare Events',
      section: 'Chart',
      order: 1
    },
  ...
  updateAsync(data, element, config, queryResponse, done) {
    try {
      ...
      const chartResults = calculateGChartLimits(data, valueColumn);
  ...
  _renderChart(params) {
    ...
    // Add title
    this._addTitle(svg, config.chart_title || 'g Chart - Opportunities Between Rare Events', svgWidth);
  ...
  _drawDataPoints(group, xScale, yScale, values, pointColors) {
    ...
    values.forEach((value, index) => {
      ...
      title.textContent = `Point ${index + 1}: ${value} opportunities between events`;
  ...
  _drawAxes(group, xScale, yScale, width, height) {
    ...
    yLabel.textContent = 'Opportunities Between Events';
...
if (typeof looker !== 'undefined') {
  looker.plugins.visualizations.add(GChart);
}
